...
# BLACK = (0, 0, 0)  # 게임 처음 화면을 블랙으로 설정한다는 의미
...
def drawObject(obj, x, y):     # 배경그림넣기
    global gamePad
    gamePad.blit(obj, (x, y))

def initGame():        # 게임을 초기화
    global gamePad, clock, background, fighter, missile, explosion, missileSound, gameOverSound
    pygame.init()      # pygame 라이브러리도 초기화 시킴
    gamePad = pygame.display.set_mode((padWidth, padHeight))
    pygame.display.set_caption('Pyshooting')      # 게임 제목
...
def runGame():       # 실제 게임 실행 되는 함수
    ...
    onGame = False
    while not onGame:      # 각종 이벤트를 처리하기 위해서  while문을 돌림
        ...
        pygame.display.update()  # 게임화면을 다시 업그레이드 해서 그림
